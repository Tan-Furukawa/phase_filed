  ...
  for i in range(Nx):
    for j in range(Ny):
      for l in range(Nz):
        ...
        omeg11[i,j] = (a22*a33 - a23**2) / det
        omeg12[i,j] = (-a12*a33 + a13*a23) / det
        omeg13[i,j] = (a12*a23 - a13*a22) / det
        omeg22[i,j] = (a11*a33 - a13**2) / det
        omeg23[i,j] = (-a11*a23 + a12*a13) / det
        omeg33[i,j] = (a11*a22 - a12**2) / det
