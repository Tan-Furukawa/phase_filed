...
if __name__ == "__main__":
    ...
    # feldspar
    #--------------------------------------------------
    ei0 = cp.array([0.0567, 0.0162, 0.016858, 0, 0.016896, 0])
    ...
    cm = cp.array([
        ...
        [    0,     0,     0,  9.7,     0, 35.0]
    ]) * 10.0**9 / (R * T) * v_ab * 10.0**(-30) * 6.02 * 10.0**23 / 4.0
    ...
    c_p = cp.array([
        ...
        [-26.2,  14.2, -19.5,    0,  44.2,    0],
        [    0,     0,     0,  9.7,     0, 35.0]
    ]) * 10.0**9 / (R * T) * v_or * 10.0**(-30) * 6.02 * 10.0**23 / 4.0
    ...
    # tmatx, omeg11 = green_tensor(Nx,Ny,Nz,kx,ky,kz,c_p,cm)
    # np.save("../data/tmatx_3d.npy", tmatx)
    tmatx = cp.asarray(np.load("../data/tmatx_3d64.npy"))
    # %%
...
for istep in range(1, nstep + 1): 
    ...
    #     ttime += dtime
        ...
        delsdc, s, el = solve_elasticity(Nx,Ny,Nz,tmatx,cm,c_p,ea,ei0,con,c0)
        # delsdc = 0
        ...
        if (istep % nprint == 0) or (istep == 1) or (np.mean(con)/bulk <0.99):
            ...
            # myplt3.display_3d_matrix(cp.asnumpy(con))
